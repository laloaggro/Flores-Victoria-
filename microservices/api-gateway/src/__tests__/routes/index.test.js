...
describe('API Gateway Routes', () => {
  ...
  describe('Request ID Propagation', () => {
    it('should handle requests with X-Request-ID', async () => {
      const response = await request(app).get('/').set('X-Request-ID', 'test-123');
  ...
  describe('HTTP Methods', () => {
    ...
    it('should support POST method', async () => {
      const response = await request(app).post('/auth/login').send({ email: 'test@test.com' });

      expect(response.status).toBeLessThan(500);
    });

    it('should support PUT method', async () => {
      const response = await request(app).put('/products/123').send({ name: 'Updated' });
