...
const productSchema = Joi.object({
  ...
  description: Joi.string().max(1000).optional().trim().messages({
...
const orderSchema = Joi.object({
  ...
  deliveryAddress: Joi.object({
    ...
    commune: Joi.string().required().trim().messages({
    ...
    instructions: Joi.string().max(500).optional().trim(),
  }).required(),

  paymentMethod: Joi.string().valid('webpay', 'transfer', 'cash').required().messages({
  ...
  message: Joi.string().max(300).optional().trim(),
});
