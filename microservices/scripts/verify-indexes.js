  ...
  for (const test of tests) {
    try {
      ...
      if (result && result.executionStats) {
        ...
        console.log(
          `   🎯 Índice usado: ${result.queryPlanner?.winningPlan?.inputStage?.indexName || 'COLLSCAN'}`
        );

        if (stats.totalDocsExamined > stats.nReturned * 10) {
          console.log(
            `   ⚠️  ADVERTENCIA: Examinó ${stats.totalDocsExamined / stats.nReturned}x más docs de los necesarios`
          );
        }
      } else {
        console.log(`   ⏱️  Tiempo: ${duration}ms`);
