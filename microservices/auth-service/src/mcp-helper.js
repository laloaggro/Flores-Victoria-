...
try {
  ...
  axios = null;
  logger.warn({ service: 'auth-service' }, '⚠️ axios no está disponible en este contenedor. Las llamadas a MCP serán NO-OP.');
}
...
async function registerEvent(type, payload) {
  if (!axios) return logger.warn({ service: 'auth-service' }, 'registerEvent: axios no disponible, evento no enviado');
  try {
    await axios.post(`${MCP_URL}/events`, { type, payload });
  } catch (err) {
    logger.error({ service: 'auth-service', err: err.message }, 'Error al registrar evento en MCP');
  }
}
...
async function registerAudit(action, agent, details) {
  if (!axios) return logger.warn({ service: 'auth-service' }, 'registerAudit: axios no disponible, auditoría no enviada');
  try {
    await axios.post(`${MCP_URL}/audit`, { action, agent, details });
  } catch (err) {
    logger.error({ service: 'auth-service', err: err.message }, 'Error al registrar auditoría en MCP');
  }
}
