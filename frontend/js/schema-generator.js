...
export function generateFAQSchema() {
  ...
  const faqItems = document.querySelectorAll('.faq-item, .accordion-item, [itemtype="https://schema.org/Question"]');
  if (faqItems.length === 0) return null;
  
  const questions = Array.from(faqItems).map(item => {
    // Buscar pregunta
    const questionEl = item.querySelector('.faq-question, .accordion-header, h3, h4, [itemprop="name"]');
    // Buscar respuesta
    const answerEl = item.querySelector('.faq-answer, .accordion-body, .faq-content, p, [itemprop="text"]');
