            ...
            const observer = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        const productName = card.querySelector('h2, h3, .product-name')?.textContent || 'Unknown Product';
                        const productPrice = card.querySelector('[class*="price"]')?.textContent?.replace(/[^0-9]/g, '') || '0';
