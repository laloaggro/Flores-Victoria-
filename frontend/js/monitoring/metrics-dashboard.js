...
class MetricsDashboard {
  ...
  setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.ctrlKey && e.shiftKey && e.key === 'M') {
        ...
        e.key === 'Escape' &&
        !document.querySelector('.metrics-dashboard').classList.contains('hidden')
      ) {
        this.hide();
      }
    });
  ...
  async collectPerformanceMetrics() {
    ...
    if (navigation) {
      ...
      this.metrics.performance.domContentLoaded =
    ...
    this.metrics.performance.totalResourceSize = resources.reduce(
  ...
  async collectBusinessMetrics() {
    // Form submissions
    this.metrics.business.formSubmissions = parseInt(
      localStorage.getItem('form_submissions') || '0'
    );
    ...
    const totalInteractions =
      this.metrics.business.formSubmissions +
  ...
  async collectSystemMetrics() {
    ...
    this.metrics.system.serviceWorkerActive =
    ...
    this.metrics.system.cacheHitRate =
      totalRequests > 0 ? ((cacheHits / totalRequests) * 100).toFixed(1) : '0';
  }
  ...
  updateUI() {
    ...
    this.updateMetricCard(
      'session-duration',
      Math.round(this.metrics.analytics.sessionDuration),
      's',
      30,
      120
    );
    ...
    const memoryUsagePercent =
      this.metrics.performance.memoryUsed && this.metrics.performance.memoryLimit
        ? (
            (this.metrics.performance.memoryUsed / this.metrics.performance.memoryLimit) *
            100
          ).toFixed(1)
    ...
    this.updateMetricCard(
      'sw-status',
      this.metrics.system.serviceWorkerActive ? 'Active' : 'Inactive',
      ''
    );
  ...
  updateMetricCard(metricName, value, unit, goodThreshold, warningThreshold) {
    ...
    if (typeof value === 'number' && goodThreshold !== undefined) {
      ...
      status =
        value.toLowerCase().includes('active') || value.toLowerCase().includes('online')
          ? 'good'
          : 'warning';
    }
