...
class AdvancedGA4Manager {
    ...
    trackPageEngagement() {
        ...
        const trackTimeOnPage = () => {
            ...
            if (timeSpent > 30 && timeSpent % 30 === 0) { // Cada 30 segundos
    ...
    trackBusinessEvents() {
        ...
        const productElements = document.querySelectorAll('[data-product], .product-card, .service-item');
        ...
        // Contact tracking
        const contactButtons = document.querySelectorAll('[href^="tel:"], [href^="mailto:"], .contact-btn');
        contactButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                const contactType = btn.href?.startsWith('tel:') ? 'phone' : 
                                  btn.href?.startsWith('mailto:') ? 'email' : 'general';
    ...
    extractProductData(element) {
        return {
            id: element.dataset.productId || element.dataset.id || Math.random().toString(36).substr(2, 9),
            name: element.dataset.productName || element.querySelector('h3, h2, .title')?.textContent?.trim() || 'Unknown Product',
            category: element.dataset.category || 'arreglos',
            price: parseFloat(element.dataset.price || element.querySelector('.price')?.textContent?.replace(/[^\d]/g, '') || '0'),
