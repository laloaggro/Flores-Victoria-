...
console.log('%c🌸 Flores Victoria Admin Site', 'color: #667eea; font-size: 20px; font-weight: bold;');
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        fetch('/api/auth/profile', { credentials: 'include' })
            ...
            .then(data => {
                ...
                if (headerName) headerName.textContent = data?.data?.user?.name || data?.data?.user?.email || 'Usuario';

                // Mostrar/ocultar tarjetas según rol
                const adminOnly = ['Dashboard Principal','MCP Dashboard','Panel de Control','Herramientas de Testing','Logs y Analíticas','Estado del Sistema'];
