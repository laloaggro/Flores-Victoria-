(function(){
  ...
  async function loadServices(){
    try {
      ...
      const names = Array.isArray(data.services) ? data.services : ['api-gateway','auth-service','product-service','frontend','admin-panel'];
      servicesDynamicList.innerHTML = names.map(n => `
  ...
  document.getElementById('presetSmokeAhora')?.addEventListener('click', () => runPipeline('smoke'));
  document.getElementById('presetRestartGateway')?.addEventListener('click', async () => {
    Array.from(document.querySelectorAll('input[name="svc"]')).forEach(i => i.checked = (i.value === 'api-gateway'));
  ...
  async function fetchLogs(){
    ...
    try {
      const res = await fetch(`/admin/logs?target=${encodeURIComponent(target)}&lines=${lines}`, { credentials: 'include' });
      const data = await res.json();
  ...
  async function loadProblems(){
    try {
      ...
      if (!Array.isArray(data.items)) { container.textContent = 'Sin datos.'; return; }
