...
│                                            │
│ 💡 Tip: Selecciona el ambiente para ver   │
│    los logs específicos y gestionar        │
│    servicios                               │
└────────────────────────────────────────────┘
```
...
Después del panel de control, se muestran los logs filtrados automáticamente por el ambiente seleccionado:
...
function restartAllServices(env) {
    if (!confirm(`¿Reiniciar TODOS los servicios en ${env}? Esto puede causar tiempo de inactividad.`)) return;
    
    const services = ['api-gateway', 'auth-service', 'order-service', 'payment-service', 'ai-service'];
