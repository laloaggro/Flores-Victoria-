...
async function validate() {
  ...
  for (const product of products) {
    ...
    try {
      ...
      if (stats.size > 800000) {
        sizes.ai++;
        console.log(`✅ ${product.id.padEnd(8)} ${filename.padEnd(20)} ${sizeMB.padStart(6)} MB (AI)`);
      } else {
        sizes.watermarked++;
  ...
  console.log(`✅ Imágenes encontradas:  ${found}/${products.length} (${Math.round(found/products.length*100)}%)`);
