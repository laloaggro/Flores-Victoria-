  ...
  try {
    switch (action) {
      case 'restart-gateway':
        ...
        exec('cd /home/impala/Documentos/Proyectos/flores-victoria && node api-gateway.js > logs/gateway.log 2>&1 &');
...
router.get('/logs/:service', async (req, res) => {
  ...
  try {
    const { stdout } = await execPromise(`tail -n ${lines} ${logFile} 2>/dev/null || echo "No logs available"`);
    res.json({
      ok: true,
      service,
      logs: stdout.split('\n')
