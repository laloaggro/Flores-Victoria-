...
const uri =
  ...
  'mongodb://admin:password@localhost:27017/floresvictoria?authSource=admin';
  ...
  try {
    ...
    server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        logError(
          `El puerto ${port} está ocupado. Por favor, detenga el proceso que lo está utilizando o especifique un puerto diferente.`,
          err
        );
        console.error(
          `El puerto ${port} está ocupado. Por favor, detenga el proceso que lo está utilizando o especifique un puerto diferente.`
