...
// Datos para Chile
const CHILEAN_NAMES = {
    male: ['Carlos', 'Luis', 'José', 'Manuel', 'Francisco', 'Antonio', 'Juan', 'Pedro', 'Miguel', 'Jorge', 'Roberto', 'Ricardo', 'Andrés', 'Fernando', 'Diego', 'Patricio', 'Gonzalo', 'Alejandro', 'Rodrigo', 'Cristián'],
    female: ['María', 'Ana', 'Carmen', 'Rosa', 'Teresa', 'Isabel', 'Francisca', 'Patricia', 'Gloria', 'Claudia', 'Mónica', 'Soledad', 'Alejandra', 'Marcela', 'Verónica', 'Silvia', 'Andrea', 'Carolina', 'Paulina', 'Lorena']
};

const CHILEAN_SURNAMES = ['González', 'Muñoz', 'Rojas', 'Díaz', 'Pérez', 'Soto', 'Contreras', 'Silva', 'Martínez', 'Sepúlveda', 'Morales', 'Rodríguez', 'López', 'Fuentes', 'Hernández', 'Torres', 'Araya', 'Flores', 'Espinoza', 'Valdés', 'Castillo', 'Núñez', 'Tapia', 'Reyes', 'Castro'];

const CHILEAN_CITIES = ['Santiago', 'Valparaíso', 'Concepción', 'La Serena', 'Antofagasta', 'Temuco', 'Rancagua', 'Talca', 'Arica', 'Chillán', 'Iquique', 'Los Ángeles', 'Puerto Montt', 'Calama', 'Coquimbo', 'Osorno', 'Valdivia', 'Punta Arenas', 'Copiapó', 'Quillota'];
...
async function generateTestData() {
    ...
    try {
        ...
        const tables = await runSQLAll(db, "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
        ...
        const products = await runSQLAll(db, 'SELECT product_id, name, is_service FROM products_accounting WHERE is_active = 1');
        const locations = await runSQLAll(db, 'SELECT location_id FROM inventory_locations WHERE is_active = 1');
        ...
        const customers = await runSQLAll(db, 'SELECT customer_id, name FROM customers_accounting WHERE is_active = 1');
        const activeProducts = await runSQLAll(db, 'SELECT product_id, name, selling_price FROM products_accounting WHERE is_active = 1');
        ...
        for (let i = 0; i < 520; i++) {
            ...
            // Crear factura
            const invoiceResult = await runSQL(db, `
        ...
        const paidInvoices = await runSQLAll(db, `
        ...
        const suppliersData = await runSQLAll(db, 'SELECT supplier_id, name FROM suppliers WHERE is_active = 1');
        ...
        for (const month of months) {
            // Ventas del mes
            const salesResult = await runSQLAll(db, `
        ...
        // Resumen financiero
        const financialSummary = await runSQLAll(db, `
            SELECT 
