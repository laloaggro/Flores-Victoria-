  ...
  try {
    ...
    const productsDb = new sqlite3.Database(productsDbPath, (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: 'Error al conectar con la base de datos de productos' });
      }
    });
    ...
    const usersDb = new sqlite3.Database(usersDbPath, (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: 'Error al conectar con la base de datos de usuarios' });
      }
    });
...
router.get('/activity', authenticateAdmin, async (req, res) => {
  try {
    ...
    const [recentLogins, recentUsers] = await Promise.all([getRecentLogins(), getRecentUsers()]);
