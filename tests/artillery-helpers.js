...
module.exports = {
   ...
   */
  generateRandomProduct(userContext, events, done) {
   ...
   * Log slow requests
   */
  checkSlowRequest(requestParams, response, context, ee, next) {
    const responseTime = response.timings.phases.total;
    if (responseTime > 500) {
   ...
   * Capturar errores
   */
  captureError(requestParams, response, context, ee, next) {
