...
const {
  ...
  publishMessage,
  subscribeToExchange,
} = require('../../microservices/messaging-service/src/app');
...
describe('Messaging Service', () => {
  ...
  test('debería enviar un mensaje a una cola', async () => {
    ...
    expect(amqp.connect().createChannel().assertQueue).toHaveBeenCalledWith(queue, {
      durable: true,
    });
  ...
  test('debería publicar un mensaje en un exchange', async () => {
    ...
    // Verificar que se haya llamado a los métodos correctos
    expect(amqp.connect().createChannel().assertExchange).toHaveBeenCalledWith(exchange, 'topic', {
      durable: true,
    });
