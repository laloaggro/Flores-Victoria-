...
const getPassword = () => {
  ...
  try {
    ...
    console.log('Contraseña leída correctamente desde archivo secreto:', password ? '****' : 'vacía');
    return password;
  } catch (err) {
...
if (!config.database.name) {
  throw new Error('No se encontró POSTGRES_DB ni DB_NAME. La base de datos debe estar configurada.');
}

if (!config.database.user) {
  throw new Error('No se encontró POSTGRES_USER ni DB_USER. El usuario de base de datos debe estar configurado.');
}
...
console.log(`Configuración de base de datos: host=${config.database.host}, port=${config.database.port}, database=${config.database.name}, user=${config.database.user}`);
...
module.exports = {
  ...
  connect: () => {
    ...
    console.log(`Detalles de conexión: host=${config.database.host}, port=${config.database.port}, database=${config.database.name}, user=${config.database.user}`);
