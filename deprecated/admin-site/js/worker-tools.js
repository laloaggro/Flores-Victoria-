(function () {
  ...
  async function getProduct(id) {
    try {
      ...
      outProd.textContent = `Error: ${e.message}`;
    }
  }
  ...
  async function createPromotion(name, discount, filter) {
    try {
      ...
      outPromo.textContent = `Error: ${e.message}`;
    }
  }
  ...
  document.getElementById('btnGet').addEventListener('click', () => {
    ...
    if (!id) {
      outProd.textContent = 'Ingresa un ID de producto';
      return;
    }
    getProduct(id);
  });
  document.getElementById('btnUpdate').addEventListener('click', () => {
    ...
    try {
      ...
      outProd.textContent = 'JSON inválido';
    }
  });
  ...
  document.getElementById('btnCreatePromo').addEventListener('click', () => {
    ...
    try {
      ...
      createPromotion(name, discount, filter);
    } catch {
      outPromo.textContent = 'JSON inválido';
    }
  });
})();
