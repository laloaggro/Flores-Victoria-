...
Victoria y sus soluciones. Esta guía está destinada a ayudar a los desarrolladores a resolver
rápidamente problemas frecuentes.
...
**Causa**: Sintaxis incorrecta en los parámetros del exporter. Algunos exporters requieren guiones
dobles (`--`) en lugar de guiones simples (`-`) para ciertas opciones.
...
**Problema**: Algunos contenedores aparecen como `Exit 1` o no se inician correctamente.
...
**Problema**: No se puede conectar a la base de datos MongoDB.
...
**Problema**: El servidor de desarrollo de Vite no responde correctamente.
...
**Problema**: Los recursos del frontend (CSS, JS, imágenes) no se cargan y devuelven errores 404.
...
**Problema**: Las variables de entorno no se cargan correctamente en los microservicios.
...
**Problema**: Prometheus no puede conectarse a los exporters para recopilar métricas.
...
**Problema**: No se puede acceder a la interfaz de Grafana.
...
Esta guía debe ayudar a resolver la mayoría de los problemas comunes que pueden surgir durante el
desarrollo y despliegue del proyecto Flores Victoria.
