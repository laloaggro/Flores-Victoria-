...
function printResult(testName, success, message = '', timing = null) {
  ...
  console.log(
    `  ${icon} ${colors.white}${paddedName}${colors.reset} ${statusColor}${message}${colors.reset} ${timingStr}`
  );
}
...
function printHeader(title, icon = icons.gear) {
  ...
  console.log(
    `${' '.repeat(padding)}${icon}  ${colors.bright}${title}${colors.reset}${colors.cyan}  ${icon}`
  );
...
function printBanner() {
  ...
  console.log(
    `║     ${icons.rocket}  SISTEMA DE PRUEBAS - FLORES VICTORIA  ${icons.rocket}          ║`
  );
  console.log(`║${' '.repeat(68)}║`);
  console.log(`${'═'.repeat(width)}`);
  ...
  console.log(
    `${colors.dim}  ${icons.target} Objetivo: Validación completa del sistema${colors.reset}\n`
  );
}
...
function printSummary() {
  ...
  console.log(
    `${colors.bright}${colors.white}  ${icons.chart} RESUMEN DE RESULTADOS${colors.reset}${colors.cyan}`
  );
  ...
  console.log(
    `${colors.white}  │${colors.bright}  Métrica${' '.repeat(19)}│  Valor${' '.repeat(11)}│${colors.reset}`
  );
  console.log(`${colors.white}  ├${'─'.repeat(tableWidth)}┤${colors.reset}`);
  ...
  metrics.forEach((metric) => {
    ...
    console.log(
      `${colors.white}  │  ${metric.color}${label}${colors.reset}${colors.white}│${metric.color}${value}  ${colors.reset}${colors.white}│${colors.reset}`
    );
  });
  ...
  if (stats.failed === 0) {
    ...
    console.log(
      `  ${icons.success} ${icons.success} ${icons.success}  ¡TODAS LAS PRUEBAS PASARON EXITOSAMENTE!  ${icons.success} ${icons.success} ${icons.success}  `
    );
    console.log(colors.reset);
  } else {
    console.log(`${colors.bgYellow}${colors.white}${colors.bright}`);
    console.log(
      `  ${icons.warning} ATENCIÓN: ${stats.failed} prueba(s) fallaron. Revisa los detalles arriba. ${icons.warning}  `
    );
    console.log(colors.reset);
  }
...
async function testIntegration() {
  ...
  console.log(
    `${colors.yellow}  ${icons.info} Ejecutando flujo completo: Login → Obtener productos\n${colors.reset}`
  );
